...
namespace FSpot.Exporters.Folder
{
	internal class OriginalGallery : FolderGallery
	{
		...
		internal void CreateZip ()
		{
			...
			try {
				if (Directory.Exists (SubdirPath ("mq")))
					CreateZipFile ("mq");

				if (Directory.Exists (SubdirPath ("hq")))
					CreateZipFile ("hq");
