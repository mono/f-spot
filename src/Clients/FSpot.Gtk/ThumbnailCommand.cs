...
namespace FSpot
{
	public class ThumbnailCommand
	{
		...
		public bool Execute (IPhoto[] photos)
		{
			...
			if (photos.Length > 1) {
				progress_dialog = new ProgressDialog (Catalog.GetString ("Updating Thumbnails"),
					ProgressDialog.CancelButtonType.Stop, photos.Length, parent_window);
			}

			int count = 0;
			foreach (IPhoto photo in photos) {
				if (progress_dialog != null
				    && progress_dialog.Update (string.Format (Catalog.GetString ("Updating picture \"{0}\""), photo.Name)))
					break;
