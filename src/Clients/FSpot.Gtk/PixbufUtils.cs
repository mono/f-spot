...
namespace FSpot
{
	public static class PixbufUtils
	{
		...
		public static Pixbuf ErrorPixbuf {
			get {
				if (error_pixbuf == null)
					error_pixbuf = GtkUtil.TryLoadIcon (FSpotConfiguration.IconTheme, "FSpotQuestionMark", 256,
		...
		public static double Fit (Pixbuf pixbuf, int destWidth, int destHeight, bool upscaleSmaller,
		                          out int fitWidth, out int fitHeight)
		{
			return Fit (pixbuf.Width, pixbuf.Height, destWidth, destHeight, upscaleSmaller,
			            out fitWidth, out fitHeight);
		}

		public static double Fit (int origWidth, int origHeight, int destWidth, int destHeight,
			bool upscaleSmaller, out int fitWidth, out int fitHeight)
		{
		...
		public class AspectLoader
		{
			...
			private void HandleSizePrepared (object obj, SizePreparedArgs args)
			{
				...
				double scale = Fit (args.Width, args.Height, max_width, max_height, true, out scale_width,
					out scale_height);
		...
		unsafe public static byte[] Pixbuf_GetRow (byte* pixels, int row, int rowstride, int width, int channels,
			byte[] dest)
		{
		...
		unsafe public static void Pixbuf_SetRow (byte* pixels, byte[] dest, int row, int rowstride, int width,
			int channels)
		{
		...
		public static Pixbuf Blur (Pixbuf src, int radius, ThreadProgressDialog dialog)
		{
			...
			for (int i = 0; i < src.Height; i++) {
				GaussianBlurEffect.RenderBlurEffect (sourceSurface, destinationSurface,
					new[] {new Gdk.Rectangle (0, i, src.Width, 1)}, radius);
